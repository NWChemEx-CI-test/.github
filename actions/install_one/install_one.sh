...
docker build -t base-${package_name}:latest -f Dockerfile/base-${package_name}.Dockerfile . --build-arg GCC_VERSION=${gcc_version} --build-arg UBUNTU_VERSION=${ubuntu_version} --build-arg GCC_VERSION=${gcc_version} --build-arg MADNESS_VERSION=${madness_version} --build-arg CATCH2_VERSION=${catch2_version} --build-arg SPDLOG_VERSION=${spdlog_version} --build-arg CEREAL_VERSION=${cereal_version} --build-arg PARENT_PACKAGE_NAME=${parent_package_name}
